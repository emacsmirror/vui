...
- Experimental incremental rendering behind =vui-incremental-render= (default off, #82): when the root renders a flat content container (fragment or unindented vstack of text), a re-render patches only the changed segments instead of erasing and rebuilding. Off by default and not yet a clear win - it speeds up stable-content cases but regresses all-changed/reorder, because the render function still rebuilds the whole vnode tree each render; see =.context/incremental-findings.md=.
