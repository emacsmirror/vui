...
- Experimental incremental rendering behind =vui-incremental-render= (default off, #82). Two parts: (1) a whole-tree skip - when a re-render yields the same vtree object as last time (what =should-update= returning nil and memoization produce), commit does nothing, making such re-renders O(1) instead of a full rebuild; (2) a segment patch for flat content containers (fragment or unindented vstack of text) that rewrites only changed lines. The whole-tree skip is a clean win; the segment patch is off by default because it only speeds stable-content cases while regressing all-changed/reorder, since the render function still rebuilds the whole vnode tree each render (see #82).
