...
- Experimental incremental rendering behind =vui-incremental-render= (default off, #82). Three parts: (1) a whole-tree skip - when a re-render yields the same vtree object as last time (what =should-update= returning nil and memoization produce), commit does nothing, O(1) instead of a full rebuild; (2) a segment patch for flat content containers (fragment or unindented vstack of text) that rewrites only changed lines; (3) a component-list bailout - when the root renders a flat vstack/fragment of component children, a re-render skips each child that opted in with =:should-update= and reports no change (and whose consumed context values are unchanged), leaving its buffer region, widgets, and state in place; changed children patch in place, reorder reuses instances by key. This breaks the per-instance commit floor: a one-item change in a 2000-component list drops from ~350ms to ~12ms. The whole suite passes with the flag on as well as off (full parity); the flag stays off pending broader coverage (nested containers, providers above a list - see #82).
