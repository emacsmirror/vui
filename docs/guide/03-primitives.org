...
All interactive elements (buttons, fields, checkboxes, selects) support keyboard
navigation with =TAB= / =S-TAB= to move between them.

** =vui-button= — Clickable Button

Creates a button that responds to user activation (=RET= or mouse click).
Buttons are rendered with brackets: =LABEL= becomes =[LABEL]=.
...
When =:max-width= is set and the button label (including brackets) exceeds that
width, the label is truncated with "...":
