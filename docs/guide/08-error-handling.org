...
(setq vui-lifecycle-error-handler
      (lambda (err hook-name component)
        (message "Error in %s.%s: %S" component hook-name err)
...
(setq vui-event-error-handler
      (lambda (err event-name component)
        (message "Event error: %S" err)))
#+end_src
