...
vui.el provides robust error handling to prevent crashes and help with debugging. Errors in render functions, lifecycle hooks, and event handlers are all caught and handled gracefully.
...
(vui-error-boundary
 ...
 :fallback (lambda (_err)
             (vui-text "Something went wrong!" :face 'error))
 :children (vui-component 'possibly-broken))
#+end_src

If =possibly-broken= throws an error during render, the fallback is shown instead of crashing.
...
| =:fallback= | Function =(lambda (error) vnode)=      |
| =:on-error= | Optional callback when error is caught |
| =:children= | Protected content                      |

** Displaying Error Details

Since =:fallback= receives the error, you can display it:
...
(vui-error-boundary
 ...
 :fallback (lambda (err)
             (vui-vstack
              (vui-text "Error occurred:" :face 'error)
              (vui-text (format "%S" err) :face 'shadow)))
 :children (vui-component 'risky-component))
#+end_src

** With Error Callback

Use =:on-error= for side effects like logging:

#+begin_src elisp
(vui-error-boundary
 :id "logged-boundary"
 :fallback (lambda (_err)
             (vui-text "Error! Check logs."))
 :on-error (lambda (err)
...
(vui-fragment
 ...
 (vui-error-boundary
  :id "protected"
  :fallback (lambda (_err) (vui-text "Protected error"))
  :children
  ...
  (vui-error-boundary
   ...
   :fallback (lambda (err)
               (vui-vstack
                ...
                (vui-text (format "Error: %S" err) :face 'shadow)
                (vui-button "Reload"
                  :on-click (lambda ()
...
Handle errors explicitly when appropriate:
  ...
  (fetch-data
   (vui-async-callback (result)
     (if (plist-get result :error)
         (vui-set-state :error (plist-get result :error))
       (vui-set-state :data (plist-get result :data)))))
