...
#+begin_src elisp
(vui-defcomponent my-component (prop1 prop2)   ; Props
  :state ((local-value "initial"))          ; State
  :render                                   ; Render function
  (vui-text (format "%s - %s" prop1 local-value)))
#+end_src

* Defining Components with =vui-defcomponent=

The =vui-defcomponent= macro is the primary way to create components:
...
#+begin_src elisp
(vui-defcomponent user-profile ...)     ; Good
(vui-defcomponent UserProfile ...)      ; Works, but not idiomatic
(vui-defcomponent "user-profile" ...)   ; Error: must be a symbol
#+end_src
...
;; Usage:
(vui-component 'user-badge :name "Alice" :role "Admin" :avatar "👤")
...
#+begin_src elisp
;; DON'T do this:
(vui-defcomponent bad-example (value)
  :render
  (progn
...
;; DO this instead:
(vui-defcomponent good-example (initial-value)
...
#+begin_src elisp
(vui-defcomponent status-display ()
  :state ((status "ready")
          (count 0))
  :render
...
(vui-defcomponent pure-example ()
  :state ((value 0))
  :render
  (vui-text (number-to-string value)))

;; Bad: Side effects in render
(vui-defcomponent impure-example ()
  :state ((value 0))
  :render
  (progn
...
#+begin_src elisp
(vui-defcomponent conditional-example (logged-in user-name)
  :render
  (if logged-in
...
#+begin_src elisp
(vui-defcomponent maybe-show (show-p message)
  :render
  (when show-p
    (vui-text message)))  ; Returns nil when show-p is false
#+end_src

Components that render to =nil= are completely invisible in layout - they don't affect =vstack= or =hstack= spacing:

#+begin_src elisp
(vui-defcomponent sidebar-widget (is-journal)
  :render
  (when is-journal
    (vui-text "Journal navigation")))

(vui-defcomponent sidebar ()
  :render
  (vui-vstack :spacing 1
    (vui-component 'sidebar-widget :is-journal nil)  ; Renders nothing, no extra blank line
    (vui-text "Stats")
    (vui-component 'sidebar-widget :is-journal nil)  ; Renders nothing, no extra blank line
    (vui-text "Outline")))
;; Result: "Stats\n\nOutline" (single blank line from spacing)
#+end_src
...
(vui-defcomponent temperature-app ()
  :state ((celsius 0))
  :render
  (vui-fragment
...
(vui-defcomponent temperature-input (label value on-change)
  :render
  (vui-hstack
...
#+begin_src elisp
;; Your solution:
(vui-defcomponent collapsible-section (title)
  :state ((expanded t))
  :render
  ;; ... implement this
  )
