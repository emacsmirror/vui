...
pass props down manually at every level. It's useful for "global" data like
themes, user authentication, or localization.
...
Context values are read-only from consumers. To update context, you have two
options:
