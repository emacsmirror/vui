...
Bordered tables automatically add padding around cell content (=| value |=
instead of =|value|=) for readability.
...
For editable tables with complex state, embed components in cells:
...
See =docs/examples/05-wine-tasting.el= for a complete editable table example.
