...
:END:
Emacs's built-in =widget.el= library provides a foundation for creating
interactive UI elements in buffers. However, it has limitations when
building complex, dynamic interfaces:

- *Imperative updates*: Widgets are created and modified imperatively,
  making it hard to reason about UI state.
- *No diffing*: Changes require manual deletion and recreation of
  widgets.
- *Coupled state*: Widget state is mixed with rendering logic.
- *Limited composition*: Composing widgets into complex layouts is
  cumbersome.

vui.el aims to bring React's declarative, component-based model to
Emacs, enabling developers to build complex UIs by describing /what/
should be rendered, not /how/ to update it.

** Core Philosophy
:PROPERTIES:
:CUSTOM_ID: core-philosophy
:END:
1. *Declarative*: Describe the UI as a function of state, not as a
   sequence of mutations.
2. *Component-based*: Build complex UIs from small, reusable, composable
   pieces.
3. *Unidirectional data flow*: Data flows down through props; events
   flow up through callbacks.
4. *Predictable*: Same props + state always produces the same output.
5. *Emacs-native*: Respect Emacs conventions (point, markers, keymaps,
   faces).

** Key Differences from React
:PROPERTIES:
:CUSTOM_ID: key-differences-from-react
:END:
...
- *Interoperability* with existing Emacs facilities (keymaps, faces,
  hooks)
...
:CUSTOM_ID: non-goals
:END:
- *Full CSS-like layout*: We work within buffer constraints (monospace,
  linear text)
- *Animation*: No built-in animation system (though hooks could enable
  it)
- *Server-side rendering*: Not applicable to Emacs
- *Concurrent rendering*: Emacs is single-threaded; we use idle timers
  instead
- *Backward compatibility with widget.el*: Clean break, though we may
  wrap widgets


* Foundational Architecture
:PROPERTIES:
:CUSTOM_ID: foundational-architecture
:END:
** High-Level Overview
:PROPERTIES:
:CUSTOM_ID: high-level-overview
:END:
#+begin_example
+-----------------------------------------------------------------+
|                        User Code                                |
...
:PROPERTIES:
:CUSTOM_ID: key-design-decisions
:END:
*Decision 1: Immutable props, mutable state* - Props flow down from
parent, are read-only to child - State is owned by component, changes
trigger re-render

*Decision 2: Render functions are pure* - Given same props and state,
produce same virtual tree - No direct buffer manipulation in render

*Decision 3: Virtual tree is a simple data structure* - Easy to diff,
serialise, inspect - No circular references in the tree itself (parent
refs stored separately)

*Decision 4: Buffer is the "DOM"* - Text content + overlays + text
properties - All buffer mutations go through the renderer, never direct

*Decision 5: Build on widget.el for input primitives* - =vui-field= wraps
widget field (not raw text + overlays) - =vui-button= wraps widget
button - =vui-select= may use widget-choice internally - TAB navigation,
field editing come free from widget library - Reconciler updates widget
values rather than delete/recreate where possible - We provide:
component model, state management, layout, reconciliation - Widget.el
provides: battle-tested input handling, keyboard navigation

*Decision 6: Two-pass rendering architecture* - *Pass 1 (Measure)*:
Calculate dimensions without buffer mutation - Width of each vnode (for
tables, boxes) - [Future: Height for multi-line support in horizontal
layouts] - *Pass 2 (Render)*: Insert into buffer with known dimensions -
Even if MVP only uses pass 1 for width, the infrastructure exists for
...
:PROPERTIES:
:CUSTOM_ID: component-definition
:END:
A component definition is a template---it describes /how/ to create
instances.
...
** Component Instance
:PROPERTIES:
:CUSTOM_ID: component-instance
:END:
An instance is a live component in the tree with actual props, state,
and buffer position.
...
** Virtual Tree Nodes
:PROPERTIES:
:CUSTOM_ID: virtual-tree-nodes
:END:
The virtual tree is what render functions produce. It's a pure data
structure.
...
:PROPERTIES:
:CUSTOM_ID: component-definition-dsl
:END:
** The =defcomponent= Macro
:PROPERTIES:
:CUSTOM_ID: the-defcomponent-macro
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: state-mutation-with-let-state
:END:
...
Where =vui--state-accessor= is a generalised variable that: 1. Reads from
instance state 2. On =setf=, updates state and marks instance dirty

** Primitive Constructors
:PROPERTIES:
:CUSTOM_ID: primitive-constructors
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: input-capturing-primitives
:END:
Beyond inline-editable fields, we need components that capture input via
prompts:
...
** Hidden Component
:PROPERTIES:
:CUSTOM_ID: hidden-component
:END:
Components can be hidden without being destroyed, preserving their
state:
...
Use =vui-hidden= when: - Toggling visibility frequently (tabs,
accordions) - Component has expensive initialization - You want to
preserve user input in hidden forms


* Reconciliation Algorithm
:PROPERTIES:
:CUSTOM_ID: reconciliation-algorithm
:END:
** Overview
:PROPERTIES:
:CUSTOM_ID: overview
:END:
The reconciler's job is to take two virtual trees (old and new) and
produce the minimal set of operations to transform the buffer from the
old state to the new state.

React's reconciliation is based on two heuristics: 1. *Elements of
different types produce different trees* --- if a node type changes,
tear down the old subtree entirely 2. *Keys provide stable identity* ---
children with the same key are the "same" element across renders

For Emacs, we add a third concern: 3. *Preserve cursor position* ---
track which component owns point and restore it after mutations

** Algorithm Structure
:PROPERTIES:
:CUSTOM_ID: algorithm-structure
:END:
#+begin_example
...
:PROPERTIES:
:CUSTOM_ID: detailed-reconciliation-logic
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: reconciling-same-type-components
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: reconciling-children-lists-the-key-algorithm
:END:
This is where keys become critical. Without keys, inserting at the
beginning of a list would cause all children to be "replaced".
...
:PROPERTIES:
:CUSTOM_ID: reconciling-primitives
:END:
...
** Operation Ordering
:PROPERTIES:
:CUSTOM_ID: operation-ordering
:END:
Operations must be applied in the correct order to maintain valid buffer
positions:
...
:END:
- *Best case (no changes)*: O(n) where n is tree depth (just prop
  comparison)
- *Typical case*: O(n) where n is number of changed nodes\\
- *Worst case (all keys changed)*: O(n) where n is total nodes

The key insight from React: by assuming different types produce
different trees, we avoid the O(n³) tree-edit-distance algorithm.


* Cursor Preservation
:PROPERTIES:
:CUSTOM_ID: cursor-preservation
:END:
** The Problem
:PROPERTIES:
:CUSTOM_ID: the-problem
:END:
In React/browser contexts, focus management is relatively simple ---
elements have a =focus()= method and the browser tracks which element is
focused. In Emacs, *point* (the cursor position) is the primary
interaction mechanism, and it's always somewhere in the buffer.

When we re-render components: - Text may be inserted before point ->
point should stay on the same logical content - Text may be deleted ->
point might need to move to a valid position\\
- The component containing point might be replaced -> we need to restore
to equivalent position - The component containing point might be deleted
entirely -> we need a fallback

** Goals
:PROPERTIES:
:CUSTOM_ID: goals-1
:END:
1. *Semantic preservation*: Point should stay on the "same thing" after
   re-render
2. *Field continuity*: If editing a field, don't disrupt the editing
   experience
3. *Graceful degradation*: If exact position is impossible, choose
   sensible fallback
4. *No flicker*: User should not perceive point jumping around

** Cursor Context Structure
:PROPERTIES:
:CUSTOM_ID: cursor-context-structure
:END:
...
:PROPERTIES:
:CUSTOM_ID: capturing-cursor-context
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: restoring-cursor-context
:END:
After mutations are applied, we restore point using the captured
context.

*Key principle*: When the component owning cursor is destroyed, place
cursor at the position where that component /was/ --- i.e., at the start
of where the destroyed component lived within its parent. This feels
...
:PROPERTIES:
:CUSTOM_ID: special-handling-for-editable-fields
:END:
...
:PROPERTIES:
:CUSTOM_ID: mark-and-region-preservation
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: integration-with-render-cycle
:END:
...
- *No absolute positioning*: Can't place text at arbitrary x,y
  coordinates
- *No overlap*: Text can't overlap (overlays can, but not content)
- *Line-based*: Vertical positioning is in lines, not pixels

However, we can achieve flexible layouts using: - Whitespace for
horizontal spacing - Newlines for vertical separation - Display
properties for advanced alignment - Tables (using =org-table= or custom
formatting)

** Layout Primitives
:PROPERTIES:
:CUSTOM_ID: layout-primitives
:END:
*** Render Context
:PROPERTIES:
:CUSTOM_ID: render-context-1
:END:
Layout primitives need access to a render context that tracks the
...
:PROPERTIES:
:CUSTOM_ID: horizontal-stack-h-stack
:END:
...
:PROPERTIES:
:CUSTOM_ID: vertical-stack-v-stack
:END:
...
:PROPERTIES:
:CUSTOM_ID: box-fixed-width-container
:END:
...
*Layout behaviour*: - If only =:width= specified: fixed width, truncate
overflow - If =:min-width= specified: auto-size to content, but at least
this wide - If =:max-width= specified: auto-size but truncate beyond
this - If both =:min-width= and =:max-width=: auto-size within range

*Re-measurement*: By default, tables re-measure all cell widths on every
render. This ensures the layout adapts when content changes (e.g., a
score updates from "4.1" to "4.12", or a name gets longer).

*Static optimisation*: For tables with fixed content, =:static t= skips
re-measurement and reuses cached column widths. Use when you know
content won't change size.
...
:PROPERTIES:
:CUSTOM_ID: spacing-and-alignment-utilities
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: layout-edge-cases-and-constraints
:END:
*** Multi-line Content in Horizontal Layouts
:PROPERTIES:
:CUSTOM_ID: multi-line-content-in-horizontal-layouts
:END:
When a child in =h-stack= contains newlines, the layout breaks. We have
options:
...
*Decision*: For MVP, we forbid newlines in h-stack children. Use v-stack
...
*** Text Wrapping
:PROPERTIES:
:CUSTOM_ID: text-wrapping
:END:
vui.el does *not* handle text wrapping. Content that exceeds window
width will either: - Extend beyond visible area (if =truncate-lines= is
t) - Wrap at window edge (if =truncate-lines= is nil)

For controlled wrapping, use =fill-region= on content before passing to
components, or implement a =vui-wrapped-text= component that pre-wraps.

*** Read-Only Regions
:PROPERTIES:
:CUSTOM_ID: read-only-regions
:END:
...
:PROPERTIES:
:CUSTOM_ID: focus-and-navigation
:END:
** Keyboard Navigation
:PROPERTIES:
:CUSTOM_ID: keyboard-navigation
:END:
Users expect TAB/S-TAB to move between interactive elements (fields,
buttons):
...
*Limitation*: Undo will restore buffer text but not component state.
After undo, the UI may be inconsistent. Options:

1. *Disable undo* in UI buffers (=buffer-undo-list= = t)
2. *State snapshots*: Store state before changes, restore on undo
   (complex)
3. *Accept inconsistency*: Document that undo may break UI (pragmatic)

For MVP, we disable undo in UI buffers and provide explicit undo-like
actions (e.g., "Undo last change" button).


** Buffer Narrowing
:PROPERTIES:
:CUSTOM_ID: buffer-narrowing
:END:
Our markers should work correctly with narrowing, but rendering assumes
full buffer access:
...
:PROPERTIES:
:CUSTOM_ID: multiple-roots-interaction
:END:
When multiple independent UI trees exist in one buffer:

1. *Non-overlapping regions*: Each root has exclusive markers; they
   shouldn't overlap
2. *Independent updates*: Re-rendering one root doesn't affect others
3. *Potential issue*: Inserting text in one tree shifts positions of
   later trees
...
:PROPERTIES:
:CUSTOM_ID: state-model-overview
:END:
...
:PROPERTIES:
:CUSTOM_ID: local-state-implementation
:END:
Each component instance has a state slot. Changes to state mark the
instance dirty and schedule a re-render.
...
:PROPERTIES:
:CUSTOM_ID: derived-state-computed-values
:END:
Sometimes state is derived from props or other state. We provide a
memoisation helper:
...
:PROPERTIES:
:CUSTOM_ID: effects-side-effects-on-state-change
:END:
...
:PROPERTIES:
:CUSTOM_ID: refs-mutable-references-without-re-render
:END:
...
:PROPERTIES:
:CUSTOM_ID: callbacks-stable-function-references
:END:
...
:PROPERTIES:
:CUSTOM_ID: state-update-batching
:END:
...
:PROPERTIES:
:CUSTOM_ID: reducer-pattern-complex-state-logic
:END:
...
:PROPERTIES:
:CUSTOM_ID: context-data-structures
:END:
#+begin_src elisp
(cl-defstruct (vui-context (:constructor vui-context--create))
...
:PROPERTIES:
:CUSTOM_ID: context-change-detection
:END:
...
:PROPERTIES:
:CUSTOM_ID: optimising-context-updates
:END:
...
:PROPERTIES:
:CUSTOM_ID: context-composition-pattern
:END:
...
:PROPERTIES:
:CUSTOM_ID: example-theme-context
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: context-vs-props-guidelines
:END:
Use *props* when: - Data is needed by immediate children - Explicit data
flow is clearer - Component reusability is important

Use *context* when: - Data is needed by many components at different
levels - Data changes infrequently (theme, locale, auth) - Prop drilling
would be excessive (>3 levels)
...
:PROPERTIES:
:CUSTOM_ID: hook-definitions-in-defcomponent
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: scheduler-and-batching
:END:
** The Problem
:PROPERTIES:
:CUSTOM_ID: the-problem-1
:END:
...
:PROPERTIES:
:CUSTOM_ID: immediate-vs-deferred-rendering
:END:
...
:PROPERTIES:
:CUSTOM_ID: priority-scheduling-advanced
:END:
...
:PROPERTIES:
:CUSTOM_ID: rendering-primitives
:END:
#+begin_src elisp
(defun vui--render-text (vnode parent-instance)
  "Render a text vnode."
...
(defun vui--render-button (vnode parent-instance)
  "Render a button vnode using widget.el push-button.

Uses widget.el's push-button widget for proper TAB/S-TAB navigation
and consistent keyboard handling across all interactive elements."
  (let* ((label (vui-vnode-button-label vnode))
         (face (or (vui-vnode-button-face vnode) 'link))
         ...
         ;; Truncate label if max-width specified
         (display-label
...
:PROPERTIES:
:CUSTOM_ID: applying-diff-operations
:END:
#+begin_src elisp
(defun vui--commit-operations (operations root-instance)
...
:PROPERTIES:
:CUSTOM_ID: instance-buffer-mapping
:END:
#+begin_src elisp
...
*Implementation Note*: The pseudocode below shows errors stored in
instance slots (=vui-instance-error=). The actual implementation uses a
global hash table =vui--error-boundary-errors= keyed by boundary ID for
simpler state management. This avoids adding an =error= slot to every
instance. Recovery is done via =vui-reset-error-boundary= which takes a
boundary ID.
...
:PROPERTIES:
:CUSTOM_ID: error-boundary-implementation
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: catching-lifecycle-errors
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: catching-effect-errors
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: event-handler-error-handling
:END:
...
:PROPERTIES:
:CUSTOM_ID: development-mode-helpers
:END:
#+begin_src elisp
...
*PROPS-SPEC*: List of
=(NAME :type TYPE :default DEFAULT :required BOOL)=

*STATE-SPEC*: List of =(NAME INITIAL-VALUE)=

** Primitive VNodes
:PROPERTIES:
:CUSTOM_ID: primitive-vnodes
:END:
...
:END:
#+begin_src elisp
;; Mount tree into a named buffer (common entry point)
(vui-mount VNODE BUFFER-OR-NAME) -> INSTANCE

;; Render into current buffer at point
(vui-render VNODE &optional BUFFER) -> INSTANCE
...
:PROPERTIES:
:CUSTOM_ID: implementation-roadmap
:END:
** Phase 1: Core Foundation (MVP) ✓
:PROPERTIES:
:CUSTOM_ID: phase-1-core-foundation-mvp
:END:
*Goal*: Render static component trees with basic interactivity

*1.1 Data Structures* - [X] Define all cl-defstructs (vnode types,
instance, context, scheduler) - [X] Component registry (hash-table
mapping name -> definition) - [X] Basic =defcomponent= macro (props and
state)

*1.2 Initial Rendering* - [X] Render primitives: text, button, newline,
space - [X] Fragment support - [X] Marker-based region tracking
(start/end per instance) - [X] Overlay creation for buttons - [X]
Read-only text protection

*1.3 Basic Interactivity* - [X] Button click handling via widget.el - [X]
Field editing with on-change callback - [X] Checkbox and select widgets - [X]
TAB/S-TAB navigation between interactive elements (via widget.el)

*1.4 Simple State* - [X] =:state= in defcomponent - [X] =vui-set-state= for
state mutation triggering re-render - [X] Full tree re-render

*Deliverable*: Can render a static form with buttons and fields that
respond to user input.


** Phase 2: Reconciliation ✓
:PROPERTIES:
:CUSTOM_ID: phase-2-reconciliation
:END:
*Goal*: Efficient updates via diffing

*2.1 Virtual Tree Diffing* - [X] Implement =vui--reconcile-component= - [X]
Type comparison logic - [X] Key-based child matching - [X] Index-based
fallback matching

*2.2 Commit Phase* - [X] Clear and re-render approach with widget reuse -
[X] Unmount callbacks for removed children - [X] Efficient widget.el
integration

*2.3 Cursor Preservation* - [X] Widget index tracking before mutations -
[X] Restore cursor to same widget index after render - [X] Fallback to
buffer start if widget removed

*Deliverable*: Can add/remove items from a list without full re-render;
cursor stays in place.


** Phase 3: Advanced State ✓
:PROPERTIES:
:CUSTOM_ID: phase-3-advanced-state
:END:
*Goal*: Full state management capabilities

*3.1 Effects System* - [X] =use-effect= implementation - [X] Dependency
array tracking - [X] Cleanup function support - [X] Effect execution
after commit

*3.2 Additional Hooks* - [X] =use-ref= (mutable reference without
re-render) - [X] =use-callback= (memoised callbacks) - [X] =use-memo=
(memoised computed values) - [X] =use-callback*= and =use-memo*= with
custom comparison modes

*3.3 Context System* - [X] =defcontext= macro - [X] Provider vnode and
rendering - [X] Context consumption via auto-generated =use-NAME= - [X]
Context change propagation to consumers

*Deliverable*: Can build components with side effects, shared state, and
deep data passing.


** Phase 4: Layout & Polish ✓
:PROPERTIES:
:CUSTOM_ID: phase-4-layout-polish
:END:
*Goal*: Production-ready features

*4.1 Layout Primitives* - [X] =vui-hstack= with spacing - [X] =vui-vstack=
with spacing and indent - [X] =vui-box= with fixed width and alignment -
[X] =vui-table= with headers, borders (ascii/unicode), column alignment -
[X] =vui-list= for dynamic list rendering with key function

*4.2 Lifecycle Hooks* - [X] =on-mount= (after first render) - [X]
=on-update= (after re-render, with prev props/state) - [X] =on-unmount=
(before removal) - [X] Proper hook ordering (children first for unmount)

*4.3 Error Handling* - [X] =vui-error-boundary= component - [X] Lifecycle
error catching with configurable handlers - [X] Event handler error
wrapping - [X] =vui-reset-error-boundary= for recovery

*4.4 Scheduler Improvements* - [X] =vui-batch= for grouping state
updates - [X] Idle-time rendering via =vui-idle-render-delay= - [X]
=vui-flush-sync= for immediate updates

*Deliverable*: Full-featured UI library with layouts, error handling,
and optimised updates.


** Phase 5: Optimisation & Tooling ✓
:PROPERTIES:
:CUSTOM_ID: phase-5-optimisation-tooling
:END:
*Goal*: Performance and developer experience

*5.1 Performance* - [X] =should-update= optimisation (skip re-render if
no changes) - [X] Memo comparison modes (eq, equal, custom) - [X] Render
timing instrumentation (=vui-timing-enabled=, =vui-report-timing=)

*5.2 Developer Tools* - [X] Component inspector (=vui-inspect=) -
[X] State viewer (=vui-inspect-state=) - [X] Render cycle
debugging/logging (=vui-debug-enabled=, =vui-debug-show=)

*5.3 Documentation & Examples* - [X] API documentation with docstrings -
[X] User guide (=docs/guide/=) - [X] Tutorial: building a todo app
(=docs/examples/02-todo-app.el=) - [X] Example: file browser
(=docs/examples/04-file-browser.el=) - [X] Example: form with validation
(=docs/examples/03-forms.el=)
...
This section tracks what has been implemented versus what remains from
the original design.

*** Achieved (matching design)

| Category         | Feature                                    | Status |
|------------------+--------------------------------------------+--------|
| *Core*           | =defcomponent= macro                       | ✓      |
|                  | Props and state management                 | ✓      |
|                  | =vui-set-state=                            | ✓      |
|                  | =vui-mount= / =vui-render=                 | ✓      |
| *Primitives*     | =vui-text=, =vui-button=, =vui-field=      | ✓      |
|                  | =vui-newline=, =vui-space=, =vui-fragment= | ✓      |
|                  | =vui-checkbox=, =vui-select=               | ✓      |
| *Layout*         | =vui-hstack=, =vui-vstack=, =vui-box=      | ✓      |
|                  | =vui-table= (borders, alignment)           | ✓      |
|                  | =vui-list=                                 | ✓      |
| *Hooks*          | =use-effect= with cleanup                  | ✓      |
|                  | =use-ref=, =use-callback=, =use-memo=      | ✓      |
|                  | =use-callback*=, =use-memo*= (custom cmp)  | ✓      |
| *Context*        | =defcontext= macro                         | ✓      |
|                  | Provider vnodes, =use-NAME= consumption    | ✓      |
| *Lifecycle*      | =on-mount=, =on-update=, =on-unmount=      | ✓      |
| *Optimisation*   | =should-update= predicate                  | ✓      |
|                  | =vui-batch=, =vui-flush-sync=              | ✓      |
|                  | =vui-idle-render-delay=                    | ✓      |
|                  | =vui-timing-enabled= instrumentation       | ✓      |
| *Error*          | =vui-error-boundary=                       | ✓      |
|                  | =vui-reset-error-boundary=                 | ✓      |
|                  | Event handler error wrapping               | ✓      |
| *Dev Tools*      | =vui-inspect=, =vui-inspect-state=         | ✓      |
|                  | =vui-debug-enabled=, debug logging         | ✓      |
| *Reconciliation* | Key-based matching                         | ✓      |
|                  | Cursor preservation                        | ✓      |

*** Not Implemented (deferred)

These features are described in the design but not yet implemented.
They may be added in future versions if needed.

| Feature               | Design Section      | Notes                          |
|-----------------------+---------------------+--------------------------------|
| =let-state=           | State Management    | Binding form with setf support |
| =let-memo=            | State Hooks         | Binding form for memos         |
| =use-reducer=         | Reducer Pattern     | Hook for reducer pattern state |
| =vui-hidden=          | Conditional Layout  | Hide without unmount           |
| =vui-separator=       | Example code        | Visual separator utility       |
| =vui-reset-component= | Recovery Strategies | Reset single component state   |
| =vui-reset-tree=      | Recovery Strategies | Reset entire tree              |
| Fine-grained diff ops | Diff Operations     | =vui-op-*= structs (see note)  |

*Note on Reconciliation*: The design describes fine-grained diff
operations (insert, delete, replace, move, update-props). The current
implementation uses a simpler clear-and-rerender approach with widget
preservation. This prioritises correctness and simplicity. Once the API
is mature and well-tested, fine-grained diffing can be added as an
...
:END:
*Unit Tests* - Each data structure constructor - Macro expansions -
Individual reconciliation operations

*Integration Tests* - Multi-component render scenarios - State change ->
re-render -> correct output - Cursor preservation across various
mutations

*Visual/Buffer Tests* - Render component -> verify buffer contents -
Compare before/after for operations

*Property-Based Testing* (optional) - Random trees -> reconcile -> correct
result - Random state changes -> consistent state
...
:END:
*Required (built-in)*: - =cl-lib= --- structures, loop, utilities -
=subr-x= --- when-let, if-let, thread macros
...
:PROPERTIES:
:CUSTOM_ID: appendix-a-comparison-with-widget.el
:END:
...
:PROPERTIES:
:CUSTOM_ID: appendix-b-example-applications
:END:
** B.1 Todo Application
:PROPERTIES:
:CUSTOM_ID: b.1-todo-application
:END:
#+begin_src elisp
...
:PROPERTIES:
:CUSTOM_ID: appendix-c-naming-conventions
:END:
| Prefix          | Meaning                   |
|-----------------+---------------------------|
| =vui-=          | Public API                |
...
:PROPERTIES:
:CUSTOM_ID: appendix-d-open-questions-future-work
:END:
These are areas identified during design that need further exploration
or are deferred to future versions.

** D.1 Multi-line Content in Horizontal Layouts
:PROPERTIES:
:CUSTOM_ID: d.1-multi-line-content-in-horizontal-layouts
:END:
*Current decision*: Forbid newlines in =h-stack= children.

*Future option*: Support proper column alignment where multi-line
content extends the "row height":
...
This requires two-pass rendering (measure then render) and is
significantly more complex.

** D.2 Table Multi-line Cells
:PROPERTIES:
:CUSTOM_ID: d.2-table-multi-line-cells
:END:
Similarly, tables currently assume single-line cells. Multi-line cells
would require: - Cell height calculation - Row height = max(cell
heights) - Padding shorter cells

*Decision*: Out of scope for v1. Tables are single-line per row.

** D.3 Responsive Layout
:PROPERTIES:
:CUSTOM_ID: d.3-responsive-layout
:END:
*Current approach*: Layout is fixed. If content exceeds window width, it
overflows.

*Potential enhancements*: - =h-stack :wrap t= --- wrap overflowing
children to next line - =vui-table :stretch t= --- proportionally
stretch/shrink columns - Breakpoint-based layout selection
...
*Question*: When does responsive layout recalculate? On window resize?
This would need hooks into =window-size-change-functions=.

** D.4 Undo Integration
:PROPERTIES:
:CUSTOM_ID: d.4-undo-integration
:END:
React doesn't have undo. Neither do we, initially.

Future exploration: - State snapshots on each change - Integration with
=undo-tree= - "Time travel" debugging

** D.5 Keyboard Navigation
:PROPERTIES:
:CUSTOM_ID: d.5-keyboard-navigation
:END:
*Current approach*: Rely on widget.el's overlay properties for TAB
navigation.

*If that's insufficient*: Implement =vui-forward= / =vui-backward= that
traverse instance tree for focusable components.

** D.6 Transient Integration
:PROPERTIES:
:CUSTOM_ID: d.6-transient-integration
:END:
For complex command palettes, integration with =transient.el= could
provide: - Rich keyboard-driven menus - Persisted prefixes - Better than
basic completing-read for complex choices
...
** D.7 Animation
:PROPERTIES:
:CUSTOM_ID: d.7-animation
:END:
Currently no animation support. Potential future work: - Transition on
show/hide - Smooth content changes - Uses idle timers + overlay
manipulation

** D.8 Accessibility
:PROPERTIES:
:CUSTOM_ID: d.8-accessibility
:END:
Consider: - Screen reader compatibility - High contrast mode via face
inheritance - Keyboard-only operation (already mostly there)

** D.9 Effects for Hidden Components
:PROPERTIES:
:CUSTOM_ID: d.9-effects-for-hidden-components
:END:
When a component is hidden via =vui-hidden=: - Should effects continue
running? (Current: yes) - Should they be paused? - What about effects
with visual side effects?

React's "offscreen" API has similar questions. For now, effects
continue.

** D.10 Server Rendering / Serialisation
:PROPERTIES:
:CUSTOM_ID: d.10-server-rendering-serialisation
:END:
Not applicable to Emacs in the traditional sense, but: - Could we
serialise a component tree? - Restore from serialisation? - Useful for
session persistence

** D.11 Concurrent Rendering
:PROPERTIES:
:CUSTOM_ID: d.11-concurrent-rendering
:END:
Emacs is single-threaded, so no true concurrency. However: - Could we
yield during large renders? - Interruptible reconciliation? -
Priority-based scheduling (user interactions first)?

** D.12 Memory Management
:PROPERTIES:
:CUSTOM_ID: d.12-memory-management
:END:
Long-running UIs might accumulate: - Detached overlays - Orphaned
instances - Callback closures
