...
(cl-defstruct (vui-component-def (:constructor vui-component-def--create))
  ...
  initial-state-fn        ; (lambda (props) state) — returns initial state plist
  render-fn               ; (lambda (props state) vtree) — pure function

  ;; Lifecycle hooks (all optional)
  on-mount                ; (lambda (props state)) — after first render
  on-update               ; (lambda (props state prev-props prev-state))
  on-unmount              ; (lambda (props state)) — before removal
...
(cl-defstruct (vui-vnode-field (:include vui-vnode)
  ...
  on-change               ; (lambda (new-value)) — triggers re-render
  on-submit               ; (lambda (value)) — called on RET, no re-render
  face
...
(defcomponent todo-item (text done on-toggle on-delete)
  ...
  ;; Render function — the body (required)
  :render
  (if editing
      ;; Editing mode
      (vui-hstack
...
- =children= — child vnodes passed via =:children= prop
- =prev-props= and =prev-state= — in =:on-update= and =:should-update=
...
Options:
  :spacing N  — blank lines between children (default 0)
...
Column spec properties (each column is a plist):
  ...
  :width N        — Fixed width, overrides content measurement
  :min-width N    — Minimum width, expand for content (default 1)
  :align ALIGN    — :left (default), :center, :right
  :grow BOOL      — If t, enforce :width as minimum
  :truncate BOOL  — If t, truncate overflow; if nil, use overflow separator

Table properties:
  :columns LIST   — List of column specs
  :rows LIST      — List of rows, each row is list of cell contents
...
FALLBACK is (lambda (error) vnode) — what to render on error.
ON-ERROR is optional (lambda (error)) — for logging.
...
Values:
  ...
  function — Custom (lambda (hook-name error instance))"
  ...)
...
| =(vui-button LABEL &rest PROPS)=  | Clickable button   | =:on-click=, =:face=, =:disabled=, =:max-width=, =:no-decoration= |
...
|                  | =vui-set-state= with functional updates    | ✓      |
|                  | =vui-mount=                                | ✓      |
...
|                  | =vui-list= (dynamic rendering with keys)   | ✓      |
...
|                  | =on-mount= cleanup return                  | ✓      |
| *Async*          | =vui-with-async-context=                   | ✓      |
|                  | =vui-async-callback=                       | ✓      |
| *Optimisation*   | =should-update= predicate                  | ✓      |
|                  | =vui-batch=, =vui-flush-sync=              | ✓      |
|                  | =vui-render-delay= (idle rendering)        | ✓      |
...
|                  | Cursor preservation (widget indexing)      | ✓      |
