...
React's reconciliation is based on two heuristics:

- *Elements of different types produce different trees* - if a node type changes, tear down the old subtree entirely
- *Keys provide stable identity* - children with the same key are the "same" element across renders

For Emacs, we add a third concern:

- *Preserve cursor position* - track which component owns point and restore it after mutations
...
When we re-render components:

- Text may be inserted before point -> point should stay on the same logical content - Text may be deleted -> point might need to move to a valid position
...
However, we can achieve flexible layouts using:

- Whitespace for horizontal spacing
- Newlines for vertical separation
- Display properties for advanced alignment
- Tables (using =org-table= or custom formatting)
...
- If only =:width= specified: fixed width, truncate overflow
- If =:min-width= specified: auto-size to content, but at least this wide
- If =:max-width= specified: auto-size but truncate beyond this
- If both =:min-width= and =:max-width=: auto-size within range
...
vui.el does *not* handle text wrapping. Content that exceeds window width will either:

- Extend beyond visible area (if =truncate-lines= is t)
- Wrap at window edge (if =truncate-lines= is nil)
...
*Implementation Note*: The pseudocode below shows errors stored in instance slots (=vui-instance-error=). The actual implementation uses a global hash table =vui--error-boundary-errors= keyed by boundary ID for simpler state management. This avoids adding an =error= slot to every instance. Recovery is done via =vui-reset-error-boundary= which takes a boundary ID.
...
- [X] Define all cl-defstructs (vnode types, instance, context, scheduler)
- [X] Component registry (hash-table mapping name -> definition)
- [X] Basic =defcomponent= macro (props and state)

*1.2 Initial Rendering*

- [X] Render primitives: text, button, newline, space
- [X] Fragment support
- [X] Marker-based region tracking (start/end per instance)
- [X] Overlay creation for buttons
- [X] Read-only text protection

*1.3 Basic Interactivity*

- [X] Button click handling via widget.el
- [X] Field editing with on-change callback
- [X] Checkbox and select widgets
- [X] TAB/S-TAB navigation between interactive elements (via widget.el)

*1.4 Simple State*

- [X] =:state= in defcomponent
- [X] =vui-set-state= for state mutation triggering re-render
- [X] Full tree re-render
...
*2.1 Virtual Tree Diffing*

- [X] Implement =vui--reconcile-component=
- [X] Type comparison logic
- [X] Key-based child matching
- [X] Index-based fallback matching

*2.2 Commit Phase*

- [X] Clear and re-render approach with widget reuse
- [X] Unmount callbacks for removed children
- [X] Efficient widget.el integration

*2.3 Cursor Preservation*

- [X] Widget index tracking before mutations
- [X] Restore cursor to same widget index after render
- [X] Fallback to buffer start if widget removed
...
- [X] =use-effect= implementation
- [X] Dependency array tracking
- [X] Cleanup function support
- [X] Effect execution after commit

*3.2 Additional Hooks*

- [X] =use-ref= (mutable reference without re-render)
- [X] =use-callback= (memoised callbacks)
- [X] =use-memo= (memoised computed values)
- [X] =use-callback*= and =use-memo*= with custom comparison modes

*3.3 Context System*

- [X] =defcontext= macro
- [X] Provider vnode and rendering
- [X] Context consumption via auto-generated =use-NAME=
- [X] Context change propagation to consumers
...
- [X] =vui-hstack= with spacing
- [X] =vui-vstack= with spacing and indent
- [X] =vui-box= with fixed width and alignment
- [X] =vui-table= with headers, borders (ascii/unicode), column alignment
- [X] =vui-list= for dynamic list rendering with key function

*4.2 Lifecycle Hooks*

- [X] =on-mount= (after first render)
- [X] =on-update= (after re-render, with prev props/state)
- [X] =on-unmount= (before removal)
- [X] Proper hook ordering (children first for unmount)

*4.3 Error Handling*

- [X] =vui-error-boundary= component
- [X] Lifecycle error catching with configurable handlers
- [X] Event handler error wrapping
- [X] =vui-reset-error-boundary= for recovery

*4.4 Scheduler Improvements*

- [X] =vui-batch= for grouping state updates
- [X] Idle-time rendering via =vui-idle-render-delay=
- [X] =vui-flush-sync= for immediate updates
...
- [X] =should-update= optimisation (skip re-render if no changes)
- [X] Memo comparison modes (eq, equal, custom)
- [X] Render timing instrumentation (=vui-timing-enabled=, =vui-report-timing=)

*5.2 Developer Tools*

- [X] Component inspector (=vui-inspect=)
- [X] State viewer (=vui-inspect-state=)
- [X] Render cycle debugging/logging (=vui-debug-enabled=, =vui-debug-show=)

*5.3 Documentation & Examples*

- [X] API documentation with docstrings
- [X] User guide (=docs/guide/=)
- [X] Tutorial: building a todo app (=docs/examples/02-todo-app.el=)
- [X] Example: file browser (=docs/examples/04-file-browser.el=)
- [X] Example: form with validation (=docs/examples/03-forms.el=)
...
This section tracks what has been implemented versus what remains from the original design.
...
These features are described in the design but not yet implemented. They may be added in future versions if needed.
...
*Note on Reconciliation*: The design describes fine-grained diff operations (insert, delete, replace, move, update-props). The current implementation uses a simpler clear-and-rerender approach with widget preservation. This prioritises correctness and simplicity. Once the API is mature and well-tested, fine-grained diffing can be added as an optimisation if needed.

** Testing Strategy

*Unit Tests*

- Each data structure constructor
- Macro expansions
- Individual reconciliation operations

*Integration Tests*

- Multi-component render scenarios
- State change -> re-render -> correct output
- Cursor preservation across various mutations

*Visual/Buffer Tests*

- Render component -> verify buffer contents
- Compare before/after for operations

*Property-Based Testing* (optional)

- Random trees -> reconcile -> correct result
- Random state changes -> consistent state

** Dependencies

*Required (built-in)*:

- =cl-lib=- structures, loop, utilities
...
Similarly, tables currently assume single-line cells. Multi-line cells would require:

- Cell height calculation
- Row height = max(cell heights)
- Padding shorter cells
...
- =h-stack :wrap t= - wrap overflowing children to next line
- =vui-table :stretch t= - proportionally stretch/shrink columns
...
For complex command palettes, integration with =transient.el= could provide:

- Rich keyboard-driven menus
- Persisted prefixes
- Better than basic completing-read for complex choices
...
Currently no animation support. Potential future work:

- Transition on show/hide
- Smooth content changes
- Uses idle timers + overlay manipulation
...
- High contrast mode via face inheritance
- Keyboard-only operation (already mostly there)

** D.9 Effects for Hidden Components

When a component is hidden via =vui-hidden=:

- Should effects continue running? (Current: yes)
- Should they be paused?
- What about effects with visual side effects?

React's "offscreen" API has similar questions. For now, effects continue.

** D.10 Server Rendering / Serialisation

Not applicable to Emacs in the traditional sense, but:

- Could we serialise a component tree?
- Restore from serialisation?
- Useful for session persistence

** D.11 Concurrent Rendering

Emacs is single-threaded, so no true concurrency. However:

- Could we yield during large renders?
- Interruptible reconciliation?
- Priority-based scheduling (user interactions first)?

** D.12 Memory Management

Long-running UIs might accumulate:
