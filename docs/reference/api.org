...
| =:memo=          | No       | =t= shorthand for a shallow-prop should-update |
| =:render=        | Yes      | The render expression (must return a vnode) |

=:memo t= is a shorthand for the most common =:should-update=: skip the
re-render while props are shallow-equal (=equal= on each value) and state
is unchanged, like =React.memo=.  An explicit =:should-update= always
takes precedence over =:memo=.  Note that =:children= counts as a prop,
so a memo component wrapping nested content re-renders whenever its
parent does (the children vnodes are freshly built each render); =:memo=
pays off for leaf-ish components driven purely by their own props.
...
Use this when external data has changed and cached computations should be
discarded (e.g., after a file save, database update, or user-triggered refresh).
...
Use this for periodic or speculative refreshes where data may not have changed.
Memoized values are preserved and only recompute if their dependencies change,
avoiding unnecessary expensive work.

NEW-PROPS completely replaces the instance's current props. Component state is
preserved through reconciliation.
...
As a rule of thumb:
- *Save hooks, manual refresh* → =vui-update= (data changed, recompute everything)
- *Idle timers, window focus* → =vui-update-props= (data probably unchanged, let memos skip work)
- *Internal triggers* → =vui-rerender= (just redraw with current props and state)
...
| =:face=    | face    | nil     | Applied beneath children's faces (see =vui-region=) |
| =:keymap=  | keymap  | nil     | Cascades beneath children's keymaps (see =vui-region=) |
...
;; Highlight a whole row, separators included
(vui-hstack :face 'highlight
            (vui-text "item")
...
| =:face=          | face    | nil     | Applied beneath child's faces (see =vui-region=) |
| =:keymap=        | keymap  | nil     | Cascades beneath child's keymaps (see =vui-region=) |
| =:key=           | any     | nil     | Reconciliation key                          |
...
#+begin_src elisp
(vui-flex [OPTIONS...] CHILDREN...) -> vnode
(vui-flex-item [OPTIONS...] CHILD) -> vnode
#+end_src

Horizontal row that distributes a total width among its children.

| Option     | Type            | Default       | Description                          |
|------------+-----------------+---------------+--------------------------------------|
| =:width=   | number/fn/symbol | =fill-column= | Total width; also accepts =window= or a function resolved at render time |
| =:spacing= | integer         | 1             | Spaces between children              |
| =:justify= | keyword         | :start        | :start, :center, :end, :space-between (when nothing grows) |
| =:face=    | face            | nil           | Applied beneath children's faces     |
| =:keymap=  | keymap          | nil           | Cascades beneath children's keymaps  |
| =:key=     | any             | nil           | Reconciliation key                   |

Children render at their natural width. Wrap a child in =vui-flex-item=
to give it a proportional share of the leftover width instead:

| Option  | Type | Default | Description                                 |
|---------+------+---------+---------------------------------------------|
| =:grow= | int  | 1       | Proportional weight among growing children  |

A =vui-flex-item= child is either a vnode (rendered inside a box padded
to the allotted width) or a function called with the allotted width -
which is what lets fields actually fill remaining space:
...
Flex rows subtract inherited vstack indentation from their available
width. When the children's natural widths already exceed the total,
everything renders at natural width and growers get zero. Rows are
single-line by design: multi-line children are measured by their widest
line, but the layout does not account for their extra lines.

For live resizing, pass =window= or a function as =:width= and enable
...
#+begin_src elisp
(vui-rerender-on-resize &optional BUFFER)
(vui-cancel-rerender-on-resize &optional BUFFER)
#+end_src

Re-render BUFFER's VUI instances (the =vui-mount= root and any inline
instances) whenever its window size changes. BUFFER defaults to the
current buffer. Re-renders go through the normal deferred scheduling,
so a burst of resize events coalesces into one re-render per instance.
The hook is buffer-local and dies with the buffer;
=vui-cancel-rerender-on-resize= removes it earlier.
...
Headers render in the =vui-table-header= face (inherits =bold=) and
border characters in =vui-table-border= (no attributes by default), so
both can be themed globally via =M-x customize-face= or overridden per
table with the properties above.
...
#+begin_src elisp
(vui-list ITEMS RENDER-FN [KEY-FN] &key VERTICAL INDENT SPACING) -> vnode
#+end_src
...
- KEY-FN :: =(lambda (item) key)= - extracts key (default: identity). Optional;
  a keyword in this position starts the options instead, so passing an
  explicit nil before keyword options is not required.
...
- =:face= :: Applied beneath the items' own faces (see =vui-region=)
- =:keymap= :: Cascades beneath the items' own keymaps (see =vui-region=)
...
;; With indentation
(vui-list items #'vui-text :indent 2)
...
| Property   | Type     | Description                                |
|------------+----------+--------------------------------------------|
| =:fallback= | function | =(lambda (error) vnode)= rendered on error |
| =:on-error= | function | Called once with error when first caught   |
| =:id=       | any      | Optional identifier (default: tree position) |
| =:children= | list     | Protected content                          |

When =:id= is omitted, the boundary's identity is derived from its
position in the rendered tree (stable across re-renders as long as the
tree shape around it does not change). Error state is scoped to the
mounted tree, so boundaries in different buffers never interfere and a
fresh mount starts clean.

#+begin_src elisp
(vui-error-boundary
 :id 'main-boundary
 :fallback (lambda (_err) (vui-text "Something went wrong!" :face 'error))
 :on-error (lambda (err) (log-error err))
 :children (list (vui-component 'risky-component)))
#+end_src

** vui-reset-error-boundary

#+begin_src elisp
(vui-reset-error-boundary &optional ID)
#+end_src

Reset error state for boundary ID, allowing retry. When ID is nil,
resets every boundary in the current tree. Call it from a component
callback or with the VUI buffer current.
