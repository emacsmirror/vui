...
#+begin_src elisp
(defcomponent counter (initial-value)
...
Create a clickable button. Buttons render with brackets: =LABEL= becomes =[LABEL]=.
...
#+begin_src elisp
(vui-list ITEMS RENDER-FN &optional KEY-FN &key VERTICAL) -> vnode
#+end_src
...
By default, items are rendered vertically (with newlines between them).
Use =:vertical nil= for horizontal lists.
...
;; Horizontal list - items on same line
(vui-list tags
...
Force immediate re-render, bypassing idle timers.
...
- Process sentinels (when not using =use-async=)
...
=use-async= loaders — those handle context automatically.
...
#+begin_src elisp
(use-ref INITIAL-VALUE) -> (VALUE . nil)
#+end_src
...
#+begin_src elisp
(use-callback* (DEPS...) :compare COMPARE BODY)
#+end_src

Like =use-callback= with configurable comparison.
...
#+begin_src elisp
(use-memo* (DEPS...) :compare COMPARE BODY)
#+end_src

Like =use-memo= with configurable comparison.

** use-async

#+begin_src elisp
(use-async KEY LOADER) -> plist
#+end_src
...
#+begin_src elisp
(defcontext NAME &optional DEFAULT-VALUE DOCSTRING)
#+end_src
...
#+begin_src elisp
(defcontext theme 'light "Current UI theme.")
...
| Variable               | Default | Description                        |
|------------------------+---------+------------------------------------|
| =vui-idle-render-delay= | 0.01   | Seconds before idle render (nil=immediate) |
...
| =use-ref=      | Mutable reference         |
| =use-callback= | Memoized callback         |
| =use-memo=     | Memoized value            |
| =use-async=    | Async data loading        |
